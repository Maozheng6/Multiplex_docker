        ...
        for j in range(col):
            for k in range(output.shape[0]):
                for l in range(output.shape[3]):
                    new_output[k,int(i*times):int((i+1)*times),int(j*times):int((j+1)*times),l]=output[k,i,j,l]
    return new_output
...
def do_args(arg_list, name):
    ...
    parser.add_argument("--input_image_mode", action="store", dest="input_image_mode", type=str, required=False, default='RGB',  help="input_image_mode")
    parser.add_argument("--model_mode", action="store", dest="model_mode", type=str, required=False, default='BGR',  help="model_mode")
    parser.add_argument("--skip_exists", action="store", dest="skip_exists", type=bool, required=False, default=False,  help="skip_exists")
...
def visulaize_argmax(savefolder,naip_fn,naip_tile,stain_num,argmax_map,net_output,resize_ratio,ori_img):
    ##########################################################
    img_npy_is_bgr = 1
    edges_overlay = ori_img
    preds = []
    input_ori_shape = naip_tile.shape
    argmax_visual =  np.zeros((input_ori_shape[0],input_ori_shape[1], 3), dtype=np.uint8)
    id2rgb = {
        1: (0, 0, 0),
        2: (255, 0, 0),
        3: (255, 255, 0),
    ...
    print('argmax success')
    stain_pred=[]
    overlayed=[]

    for stain_i in range(0,stain_num+1):
        argmax_visual[argmax_map==stain_i+1,:]=id2rgb[stain_i+1]
        output_classes = net_output[..., stain_i]
        ...
        #print('save heatmap')
        resize_back_heat = cv2.resize(output_classes,(int(output_classes.shape[1]/resize_ratio),int(output_classes.shape[0]/resize_ratio)))
        #cv2.imwrite(savefolder+naip_fn.split('/')[-1][0:-4]+'_'+str(stain_i)+'.png',resize_back_heat)
    ...
    f, axarr = plt.subplots(2, figsize=( 15, 12))
    if img_npy_is_bgr == 1:
        axarr[0].imshow(ori_img[:,:,::-1]/255)
    else:
        axarr[0].imshow(ori_img[:,:,:]/255)
    axarr[0].set_title('RGB')
    axarr[0].axis('off')

    axarr[1].imshow(argmax_visual)
    axarr[1].set_title('"%s"' % ('argmax'))
    axarr[1].axis('off')
    f.savefig(savefolder+os.path.basename(naip_fn)[0:-4]+'_argmax_visual.png',      bbox_inches='tight', pad_inches=0)
    plt.close(f)
    print('save argmax success')
...
def main():
    ...
    input_image_mode = args.input_image_mode
    ...
    ######################
    ############################################3
    model,output_shape,input_shape,model_input_size = load_model(model_fn)
    #############################################
    preds = []
    ...
    for i in range(len(name_list)):
        ...
        ##################################################################
        ##################################################################
        #naip_tile,input_image_mode,model_mode,resize_ratio
        #to BGR
        #naip_tile = naip_tile[:,:,::-1]
        #conver input image to BGR
        if input_image_mode == 'BGR' and model_mode == 'BGR':
            Img_npy_is_bgr = 1
            pass
        elif input_image_mode == 'BGR' and model_mode == 'RGB':
            naip_tile = naip_tile[:,:,::-1]
            img_npy_is_bgr = 0
        ...
        if flag ==None:
            continue
        ##########################################################
        visulaize_argmax(savefolder,naip_fn,naip_tile,stain_num,argmax_map,     net_output,resize_ratio,ori_img)
        ########################################
        '''
        input_ori_shape = naip_tile.shape
        argmax_visual =  np.zeros((input_ori_shape[0],input_ori_shape[1], 3), dtype=np.uint8)
