...
def main():
    ...
    for i in range(len(name_list)):
        ...
        if input_image_mode == 'BGR':
            pass
        elif input_image_mode == 'RGB':
        ...
        argmax_name = savefolder+os.path.basename(naip_fn)[0:-4]+'_argmax.npy'
        ...
        for idx in range(1+stain_num):
            if idx==0:
                axarr[idx//per][idx%per].imshow(ori_img[:,:,:]/255)
                axarr[idx//per][idx%per].set_title('RGB')
                axarr[idx//per][idx%per].axis('off')


            else:
                axarr[idx//per][idx%per].imshow(overlayed[idx-1][:,:,:]/255)
        ...
        axarr[1].axis('off')
        f.savefig(savefolder+os.path.basename(naip_fn)[0:-4]+'_argmax.png',      bbox_inches='tight', pad_inches=0)
        plt.close(f)
