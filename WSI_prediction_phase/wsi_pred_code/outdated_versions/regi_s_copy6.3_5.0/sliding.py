...
def register_single(img1_ref_bgr, img2_wsi_bgr, out_dir, slide_name, dan_tile_idx, if_regi):
    MAX_FEATURES_1 = 500
...
class SLIDING_WINDOW():
    ...
    def one_window(self,paras):
        ...
        sim_value = register_single(self.ref_img_level, window, self.regi_out_dir,self.slide_name,self.dan_tile_idx,if_regi = False)
...
if __name__ == '__main__':
    ...
    slide_name_list = ['O3936-multires','3908-multires','N22034-multires', 'O0135-multires','O3105-multires','N9430-multires']
    for slide_name in slide_name_list:
        for file_i in files:
            ...
            win_size = 4*dan_on_wsi_size
            step_size = 2*dan_on_wsi_size
            ...
            with concurrent.futures.ThreadPoolExecutor( max_workers=20) as executor:
                for number, prime in zip(sliding_list, executor.map(sliding_window.one_window, sliding_list, chunksize=10)):
            ...
            min_point = np.where(sim_matrix == np.min(sim_matrix))
            ...
            registered = register_single(ref_img, window_level_new,regi_out_dir,slide_name,dan_tile_idx, if_regi = False)
