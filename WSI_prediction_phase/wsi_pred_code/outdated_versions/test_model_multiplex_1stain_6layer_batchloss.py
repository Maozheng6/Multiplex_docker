...
def main():
    ...
    #output_base=output_base.rstrip('.csv')
    ...
    preds = []
    print(pair_list)
    iterlist=list(range(len(pair_list)))
    random.shuffle(iterlist)
    print('iterlist',iterlist[:3])
    for i in iterlist:
        ...
        if os.path.isfile(savefolder+naip_fn.split('/')[-1][0:-4]+'_'+str(4)+'_overlay.png'):
            continue
	...
	#resize !!!!!!!!!!!!!!!!!!!!!!
        #naip_tile=cv2.resize(naip_tile,(int(naip_tile.shape[1]/2),int(naip_tile.shape[0]/2)))
        channel_times=10
        ...
        for stain_i in range(stain_num):
            ...
            #resize back!!!!!
            #output_classes_1=cv2.resize(output_classes,(output_classes.shape[1]*2,output_classes.shape[0]*2))
            output_classes_1=output_classes
            cv2.imwrite(savefolder+naip_fn.split('/')[-1][0:-4]+'_'+str(stain_i)+'_pred.png',output_classes_1)


            heatmap1,thre= cv2.threshold(output_classes,60,255,cv2.THRESH_BINARY)
            kernel = np.ones((3,3),np.uint8)
