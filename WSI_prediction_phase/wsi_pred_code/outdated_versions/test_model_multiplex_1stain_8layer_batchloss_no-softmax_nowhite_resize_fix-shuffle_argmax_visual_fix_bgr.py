...
from color_trans import wsi_2_dan
# Setup
...
def run_model_on_tile(model, naip_tile, inpt_size, batch_size, out_put_times,stain_num):
    ...
    if batch_count > 0:
        print('np.array(batch).shape',np.array(batch).shape)
        model_output = model.eval(np.array(batch))
...
def main():
    ...
    for i in range(len(name_list)):
        ...
        ##############################
        cv2.imwrite(savefolder+naip_fn.split('/')[-1][0:-4]+'_ori_resized.png',naip_tile)
        ...
        stain_pred=[]
        overlayed=[]
        argmax_map = np.argmax(output,axis=2)+1
        argmax_visual =  np.zeros((height,width, 3), dtype=np.uint8)
        id2rgb = {
            1: (0, 0, 0),
            2: (255, 0, 0),
            3: (255, 255, 0),
