...
import sys
import json
binarize_thre=int(sys.argv[1])
results=sys.argv[2]
print('results',results)
files=glob.glob(input_folder+'/*.zip')


base_folder='/mnt/blobfuse/train-output/ByMZ/data_dots_labels_for_multiplex/1st_batch'
...
if __name__ == '__main__':
    ...
    precision_dict={'Black':[],'Red':[],'Yellow':[],'Cyan':[],'Purple':[]}
    recall_dict={'Black':[],'Red':[],'Yellow':[],'Cyan':[],'Purple':[]}
    for annotator in ['Emily']:#['Areeha','Christian','Emily', 'Inga']:# ['Christian','Emily']:# ['Areeha','Christian','Emily','Inga']:
        im_names+=glob.glob(os.path.join(base_folder,image_folder,annotator)+'/*.png')
