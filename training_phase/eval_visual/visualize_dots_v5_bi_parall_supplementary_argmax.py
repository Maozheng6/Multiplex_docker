...
def visualize_one_patch(folder_i,save_folder_processed_dots,imname,save_folder,colors_by_one_annotator,precision_dict, recall_dict):
    ...
    #####################################################
    #argmax process
    heat_pred=cv2.imread(os.path.join(results,os.path.basename(imname)[0:-4]+'_'+str(0)+'.png'),0)
    if heat_pred.shape[0]>400:
        heat_pred=cv2.resize(heat_pred,(400,400))
    heat_pred_stack = np.zeros((heat_pred.shape[1],heat_pred.shape[0],len(cell_type.keys())+1))
    heat_layer_count=0
    for color_num in range(len(cell_type.keys())+1):
        heat_pred=cv2.imread(os.path.join(results,os.path.basename(imname)[0:-4]+'_'+str(color_num)+'.png'),0)
        if heat_pred.shape[0]>400:
            heat_pred=cv2.resize(heat_pred,(400,400))
    ...
    for color,item in all_parsed_info_dict_sorted.items():
        ...
        print('heat_pred_argmax.shape',heat_pred_argmax.shape,color_idx[color])
        ####################################
        #argmax process
        heat_binary=np.zeros((heat_pred_argmax.shape[1],heat_pred_argmax.shape[0],3)).astype('uint8')
...
if __name__ == '__main__':
    ...
    for key in precision_dict.keys():
        ...
        F1[key]=2*precision_final[key]*recall_final[key]/(precision_final[key]+recall_final[key]+0.00001)
