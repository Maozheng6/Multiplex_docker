...
base_folder='/scratch/KurcGroup/mazhao/ICCV/data_multiplex/dots_dan/big_path_with_dots_testset_O0135/'
...
def process_one_patch_txt(folder_i,save_folder_processed_dots,imname):
    ...
    for item in all_parsed_info1:
        ...
        if item[0]=='Brown':
            all_parsed_info.remove(item)
        if  item[0]== 'K17-neg tumor' or item[0]== 'K17-NEGATIVE TUMOR' or  item[0]=='K17 NEGATIVE TUMOR' or  item[0].lower()=='k17 neg' or  item[0].lower()=='K17-negative tumor' or  item[0].lower()=='k17 - neg' or item[0].lower()=='k17-neg' or  item[0].lower()=='k17-negative' or item[0].lower()=='k17 negative tumor' or item[0].lower()=='k17-negative tumor' or  item[0].lower()== 'k17-neg tumor' or item[0].lower()=='k17-neh':
            all_parsed_info.remove(item)
...
def visualize_one_patch(folder_i,save_folder_processed_dots,imname,save_folder,colors_by_one_annotator,precision_dict, recall_dict):
    ...
    #argmax process
    '''
    print('read heat_pred',os.path.join(results,os.path.basename(imname)[0:-4]+'_'+str(0)+'.png'))
    heat_pred_name = os.path.join(results,'O0135-multires_'+os.path.basename(imname)[0:-4]+'_wsi_'+str(0)+'.png')
    heat_pred=cv2.imread(os.path.join(results,os.path.basename(imname)[0:-4]+'_resized_'+str(0)+'.png'),0)
    #if heat_pred.shape[0]>400:
    #    heat_pred=cv2.resize(heat_pred,(400,400))
    heat_pred_stack = np.zeros((heat_pred.shape[0],heat_pred.shape[1],len(cell_type.keys())+1))
    heat_layer_count=0
    for color_num in range(len(cell_type.keys())+1):
        heat_pred=cv2.imread(os.path.join(results,os.path.basename(imname)[0:-4]+'_resized_'+str(color_num)+'.png'),0)
        #if heat_pred.shape[0]>400:
        #    heat_pred=cv2.resize(heat_pred,(400,400))
        heat_pred_stack[:,:,color_num] = heat_pred
    heat_pred_argmax = np.argmax(heat_pred_stack,axis=-1)+1
    '''
    heat_pred_argmax = np.load(os.path.join(results,'O0135-multires_'+os.path.basename(imname)[0:-4]+'_dan_argmax.npy'))
    ...
    for color,item in all_parsed_info_dict_sorted.items():
       ...
       ######################
        ...
        ori_image=cv2.imread(os.path.join(base_folder,'O0135-multires_'+os.path.basename(imname)[0:-4]+'_wsi.png'))
...
if __name__ == '__main__':
    ...
    im_names=glob.glob(base_folder+'/*.tif')
    ...
    mean_recall=mean_recall/(stain_num-1)
    ...
    mean_recall_5=mean_recall_5/(len(recall_final.keys())-1)
    ...
    mean_precision=mean_precision/(stain_num-1)
    ...
    mean_precision_5 = mean_precision_5/(len(precision_dict_5.keys())-1)
    ...
    mean_F1 = mean_F1/(stain_num-1)
    print('mean_F1',mean_F1,'F1_final',F1_final)

    mean_F1_5=0
    for key in precision_dict_5.keys():
        mean_F1_5+=F1_final[key]
    mean_F1_5 = mean_F1_5/(len(precision_dict_5.keys())-1)
