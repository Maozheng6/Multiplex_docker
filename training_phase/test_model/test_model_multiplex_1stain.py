...
def run_model_on_tile(model, naip_tile, inpt_size, batch_size, out_put_times,stain_num):
    ...
    output = np.zeros((height, width, 2*stain_num), dtype=np.float32)
...
def main():
    ...
    for i in reversed(range(len(pair_list))):
        ...
        stain_colors=['CD3: yellow','CD4: cyan','CD8: purple','CD16: dark brown','CD 20: pink']
        for idx in range(1+stain_num):
            if idx==0:
                ...
                axarr[idx//3][idx%3].imshow(stain_pred[idx-1-3])
