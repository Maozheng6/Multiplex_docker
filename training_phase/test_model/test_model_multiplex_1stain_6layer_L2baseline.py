...
from L2_distance import L2_5layer, disRGBimg
from color_deconve import convert_to_optical_densities, convert_to_optical_densities_1, norm, color_deconvolution, get_deconve
# Setup
...
def run_model_on_tile(model, naip_tile, inpt_size, batch_size, out_put_times,stain_num):
    ...
    output = np.zeros((height, width, 6), dtype=np.float32)
    ...
    for y_index in (list(range(0, height - inpt_size, stride_y)) + [height - inpt_size,]):
        for x_index in (list(range(0, width - inpt_size, stride_x)) + [width - inpt_size,]):
            ...
            #model_output=L2_5layer(naip_im)
            model_output=get_deconve(naip_im)
            ...
            output[y_index:y_index+inpt_size, x_index:x_index+inpt_size] += model_output[0] * kernel[..., np.newaxis]
            counts[y_index:y_index+inpt_size, x_index:x_index+inpt_size] += kernel
...
def main():
    ...
    model_input_size = 400#input_shape[1]
    model=None
    ...
    for i in reversed(range(len(pair_list))):
        ...
        if naip_tile.shape[1]>400:
            naip_tile=cv2.resize(naip_tile,(400,400))
        ...
        for stain_i in range(stain_num):
            ...
            print('max min 255',np.max(output_classes),np.min(output_classes))
            heatmap1,thre= cv2.threshold(output_classes,225,255,cv2.THRESH_BINARY)
            kernel = np.ones((3,3),np.uint8)
        ...
        stain_colors=['CD3: yellow','CD4: cyan','CD8: purple','CD16: black','CD 20: pink']
        for idx in range(1+stain_num):
            if idx==0:
                axarr[idx//3][idx%3].imshow(ori_img[:,:,::-1]/255)
                axarr[idx//3][idx%3].set_title('RGB')
                axarr[idx//3][idx%3].axis('off')
            else:
                axarr[idx//3][idx%3].imshow(overlayed[idx-1-3][:,:,::-1]/255)
                axarr[idx//3][idx%3].set_title('"%s"' % (stain_colors[idx-1]))
                axarr[idx//3][idx%3].axis('off')
        f.savefig(savefolder+os.path.basename(naip_fn)[0:-4]+'_6fig.png', bbox_inches='tight', pad_inches=0)
        plt.close(f)
