...
def F1_two_ann_one_color(ann1,ann2,common_images,color_for_F1):
    color_i_ann1_none_union_total=[]
    color_i_ann2_none_union_total=[]
    color_i_union_ann12_total=[]
    for i in range(len(common_images)):
        #read dots for ann1
        save_folder_processed_dots_ann1 = os.path.join(base_folder,'processed_dots',ann1)
        image_name_ann1=str(common_images[i]).split('/')[-1].rstrip('\'')

        with open(save_folder_processed_dots_ann1+'/'+image_name_ann1+'-points'+'.json', 'r') as f:
           all_parsed_info_dict_sorted_ann1 = collections.OrderedDict(json.load(f))
        print('all_parsed_info_dict_sorted_ann1',all_parsed_info_dict_sorted_ann1)

        #read dots for ann2
        save_folder_processed_dots_ann2 = os.path.join(base_folder,'processed_dots',ann2)
        image_name_ann2=str(common_images[i]).split('/')[-1].rstrip('\'')
        with open(save_folder_processed_dots_ann2+'/'+image_name_ann2+'-points'+'.json', 'r') as f:
           all_parsed_info_dict_sorted_ann2 = collections.OrderedDict(json.load(f))

        print(all_parsed_info_dict_sorted_ann1,'\n',all_parsed_info_dict_sorted_ann2)

        #get the dots of this color
        if color_for_F1 not in all_parsed_info_dict_sorted_ann1.keys():
            color_i_ann1=[]
        else:
            color_i_ann1=all_parsed_info_dict_sorted_ann1[color_for_F1][1]
        if color_for_F1 not in all_parsed_info_dict_sorted_ann2.keys():
            color_i_ann2=[]
        else:
            color_i_ann2=all_parsed_info_dict_sorted_ann2[color_for_F1][1]

        color_i_ann1_none_union=copy.deepcopy(color_i_ann1)
        color_i_ann2_none_union=copy.deepcopy(color_i_ann2)
        color_i_union_ann12=[]
        distance_threshold = 10
        for location1 in color_i_ann1:
            dict_distance={}
            for location2 in color_i_ann2:
                distance=np.sqrt((location1[0]-location2[0])**2+(location1[1]-location2[1])**2)
                dict_distance[distance]=location2
            #sort distance, select the nearest
            if len(sorted(dict_distance.keys()))!=0:
                nearest_dis=sorted(dict_distance.keys())[0]
                #print('nearest',centroids[centroids_id],gt_distance[gt_nearest_dis],gt_nearest_dis)
                if nearest_dis<distance_threshold:
                    color_i_ann2.remove(dict_distance[nearest_dis])
                    color_i_ann1_none_union.remove(location1)
                    color_i_ann2_none_union.remove(dict_distance[nearest_dis])
                    color_i_union_ann12.append([location1,dict_distance[nearest_dis]])

        color_i_ann1_none_union_total+=color_i_ann1_none_union
        color_i_ann2_none_union_total+=color_i_ann2_none_union
        color_i_union_ann12_total+=color_i_union_ann12


    recall=len(color_i_union_ann12_total)/(len(color_i_union_ann12_total)+len(color_i_ann1_none_union_total)+0.001)
    precision=len(color_i_union_ann12_total)/(len(color_i_union_ann12_total)+len(color_i_ann2_none_union_total)+0.001)
    F1=2 * recall * precision/( recall + precision +0.000001)

    return (recall, precision, F1,len(color_i_union_ann12_total),len(color_i_ann1_none_union_total),len(color_i_ann2_none_union_total))
...
if __name__ == '__main__':
    common_images=np.load('common_imgs_names.npy')
    ann1='Christian'
    ann2='Emily'
    color_for_F1='Cyan'
    annotators=['Christian','Emily']#,'Areeha','Inga']
    F1_matrix=np.zeros((2,2))
    sum_list=[]
    for m1 in range(len(annotators)):
        for m2 in range(len(annotators)):
            ann1=annotators[m1]
            ann2=annotators[m2]
            out=F1_two_ann_one_color(ann1,ann2,common_images,color_for_F1)
            print(out)
            F1_matrix[m1,m2]=out[2]
            if m2>m1:
                sum_list.append(out[2])

    print(F1_matrix,len(sum_list),mean(sum_list))
