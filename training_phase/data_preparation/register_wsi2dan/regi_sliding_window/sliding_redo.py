...
#whole slide size
    ...
    save_folder = './cropped_imgs_wd4_st_1_redo2'
    if save_img==True:
...
def register_single(img1_ref_bgr, img2_wsi_bgr, out_dir, slide_name, dan_tile_idx,save_ratio, if_regi):
    ...
    if if_regi != True :
        ...
        retioed_size= np.array((width*save_ratio, height*save_ratio)).astype('int')
        img2Reg = cv2.warpPerspective(img2, h, (retioed_size[0],retioed_size[1]))
...
if __name__ == '__main__':
    ...
    level=3
    save_folder = './sliding_windows_redo2/'
    regi_out_dir = './regi_out_redo2_refined/'
    slide_results = './slide_results_redo2/'
    ...
    for slide_name in wsi2dan_slide.keys():
        ref_folder = './redo/'
        files = glob.glob(ref_folder+slide_name+'*png')
        print(files)
        oslide = openslide.OpenSlide(wsi_path+slide_name+'.tif')
        ...
        for file_i in files:
            ...
            dan_tile_idx = os.path.basename(file_i).split('_')[1]+'_'+ os.path.basename(file_i).split('_')[2]
            ...
            win_size = 3*dan_on_wsi_size
            step_size = (0.5*dan_on_wsi_size).astype(int)
            ...
            top_left_col = min_point[1][0]*step_size[0]-2*step_size[0]
            top_left_row = min_point[0][0]*step_size[1]-2*step_size[1]
            ...
            if 1:
                ...
                registered = register_single(ref_img, window_level_new,regi_out_dir,slide_name,dan_tile_idx, 1.0, if_regi = True)
                print('window_level_new.shape',window_level_new.shape)
